...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    ax.axis('off')
    ax.set_title(title)
    #cv2.imwrite("orig.png",image.astype(np.uint8))
    ...
    cv2.imwrite('splash1.png', masked_image.astype(np.uint8))
    if detect:
        plt.close()
        return canvas
    ...
    if making_video or real_time:
        ...
        import time
        cv2.imwrite('splash{}.png'.format(time.time()), X)
        return X
        #cv2.imwrite('original.png',image.astype(np.uint8))
    #if auto_show:
        #plt.show()
